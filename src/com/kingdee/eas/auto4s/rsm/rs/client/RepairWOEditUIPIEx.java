...
import com.kingdee.eas.auto4s.bdm.rsm.client.RepairItemEntryListUIPIEx;
...
public class RepairWOEditUIPIEx extends RepairWOEditUI {
	...
	private HashMap<Integer, Integer> hashOrginalLockIndex = new HashMap<Integer, Integer>(); 
	...
	public RepairWOEditUIPIEx() throws Exception {
		...
		kdtRWOItemSpEntry_repairWay_ComboBox
				.setName("kdtRWOItemSpEntry_repairWay_ComboBox");
		kdtRWOItemSpEntry_repairWay_ComboBox.setVisible(true);
		kdtRWOItemSpEntry_repairWay_ComboBox.addItem(RepairWayEnum.DEFAULT);
		kdtRWOItemSpEntry_repairWay_ComboBox
				.addItem(RepairWayEnum.REPAIRSUPPLY);
		kdtRWOItemSpEntry_repairWay_ComboBox.addItem(RepairWayEnum.GROUPBY);
		KDTDefaultCellEditor kdtRWOItemSpEntry_repairWay_CellEditor = new KDTDefaultCellEditor(
				kdtRWOItemSpEntry_repairWay_ComboBox);
		kdtRWOItemSpEntry.getColumn("repairWay").setEditor(
				kdtRWOItemSpEntry_repairWay_CellEditor);
	...
	private void getLineOprtPermission() throws Exception {
		...
		ServiceOrgUnitInfo seriveOrgInfo = (ServiceOrgUnitInfo) prmtOrgUnit
				.getValue();
		hasPermission_OprtRetailLine = PermUtils.hasFunctionPermission(null,
				userInfo, seriveOrgInfo, "oprtRetailLine");
		hasPermission_OprtRepairLine = PermUtils.hasFunctionPermission(null,
				userInfo, seriveOrgInfo, "oprtRepairLine");
		hasPermission_OprtRetailItemspName = PermUtils.hasFunctionPermission(
				null, userInfo, seriveOrgInfo, "oprtRetailItemspName");
		hasPermission_OprtRetailPrice = PermUtils.hasFunctionPermission(null,
				userInfo, seriveOrgInfo, "oprtRetailPrice ");
		hasPermission_OprtRetailBelowCost = PermUtils.hasFunctionPermission(
				null, userInfo, seriveOrgInfo, "oprtRetailBelowCost");
		hasPermission_OprtRepairTaxPrice = PermUtils.hasFunctionPermission(
				null, userInfo, seriveOrgInfo, "oprtRepairTaxPrice");
	...
	private void getUserType() throws Exception {
		...
		String sql = "select CFUserType from T_PM_User where FID='"
				+ curUserInfo.getString("id") + "'";
		try {
		...
		if (curUserType == null) {
			MsgBoxEx.showInfo(String.format("当前用户[%s]必须先设置账户类别,否则不能使用维修工单！",
					curUserInfo.getName()));
			abort();
		}
	}
	...
	protected void lockUIForViewStatus() {
		...
		if (PublicUtils.equals(repairWay.getSelectedItem(),
				RepairWayEnum.REPAIRSUPPLY)) {
	...
	public void onLoad() throws Exception {
		...
		for (int i = 0; i < kdtRWOItemSpEntry.getColumnCount(); i++) {
			if (kdtRWOItemSpEntry.getColumn(i).getStyleAttributes().isLocked()) {
				hashOrginalLockIndex.put(i, i);
			}
		}
		

		// 给[项目/配件明细]的小行操作按钮重新绑定action
		KDWorkButton btnAddNewLine = ((DetailPanel) kdtRWOItemSpEntry_detailPanel)
				.getAddNewLineButton();
		KDWorkButton btnInsertLine = ((DetailPanel) kdtRWOItemSpEntry_detailPanel)
				.getInsertLineButton();
		KDWorkButton btnRemoveLines = ((DetailPanel) kdtRWOItemSpEntry_detailPanel)
				.getRemoveLinesButton();
		...
		String sql = String
				.format(
						"select isnull(CFMaxRepairDiscountRate,0) maxRepairDiscountRate,"
								+ "isnull(CFMaxRetailDiscountRate,0) maxRetailDiscountRate from T_PM_User where FID='%s'",
		...
		if (brandInfo != null) {
			...
			defaultRepairItemForTXT = GAUtils.getDefaultRepairItemForTXT(null,
					brandInfo, orgUnitInfo);
			// defaultRepairItemForDJQ =
			// GAUtils.getDefaultRepairItemForDJQ(null, brandInfo,orgUnitInfo);

			// 按品牌获取默认维修类型与维修种类
			defaultRepairTypeInfo = GAUtils.getDefaultRepairType(null,
					brandInfo, orgUnitInfo);
		}

		KDBizPromptBox prmtPersonEntry = (KDBizPromptBox) kdtRWOItemSpEntry
				.getColumn("person").getEditor().getComponent();
		prmtPersonEntry.setCommitFormat("$number$;$name$");

		KDBizPromptBox prmtSupplierEntry = (KDBizPromptBox) kdtRWOItemSpEntry
				.getColumn("supplier").getEditor().getComponent();
		prmtSupplierEntry.setCommitFormat("$number$;$name$");
		RsQueryF7Utils.makeSupplierF7(userInfo, prmtSupplierEntry,
				(ServiceOrgUnitInfo) prmtOrgUnit.getValue());

		KDBizPromptBox prmtGiftDeptEntry = (KDBizPromptBox) kdtRWOItemSpEntry
				.getColumn("giftDept").getEditor().getComponent();
		EntityViewInfo entityGiftDept = new EntityViewInfo();
		FilterInfo filterGiftDept = new FilterInfo();
		filterGiftDept.getFilterItems().add(
				new FilterItemInfo("number", orgUnitInfo.getNumber() + "%",
						CompareType.LIKE));
		entityGiftDept.setFilter(filterGiftDept);
		prmtGiftDeptEntry.setEntityViewInfo(entityGiftDept);
		...
		filterCustomerAccount.getFilterItems().add(
				new FilterItemInfo("orgUnit.id", orgUnitInfo.getString("id")));
		filterCustomerAccount.getFilterItems().add(
				new FilterItemInfo("deletedStatus", 1));
		entityCustomerAccount.setFilter(filterCustomerAccount);
		prmtCustomerAccount.setEntityViewInfo(entityCustomerAccount);

		KDBizPromptBox prmtSAEntry = (KDBizPromptBox) kdtRWOItemSpEntry
				.getColumn("person").getEditor().getComponent();
	...
	@Override
	protected void makeSupplierF7() {
		super.makeSupplierF7();
		prmtSupplier.setCommitFormat("$number$;$name$");
	}
	...
	private void defaultValueForAddNew() throws Exception {
		if (getOprtState().equals(OprtState.ADDNEW)) {
			VehicleInfo defaultVehicleInfo = GAUtils.getDefualtVehicleInfo(
					null, orgUnitInfo);
			prmtVehicle.setValue(defaultVehicleInfo);

			CustomerAccountInfo defaultCustomerAccountInfo = GAUtils
					.getDefaultCustomerAccountInfo(null, orgUnitInfo);
			prmtCustomerAccount.setValue(defaultCustomerAccountInfo);

			/** 上次里程+1 */
			String sql = String
					.format(
							"select isnull(max(FMile),0)+1 from T_ATS_RepairWO where FVehicleID='%s'",
							defaultVehicleInfo.getString("id"));
			IRowSet rs = DBUtils.executeQuery(null, sql);
			if (rs != null && rs.next()) {
			...
			RepairWOBizTypeInfo defaultBizType = GAUtils
					.getDefaultBizType(null);
			prmtrepairBizType.setValue(defaultBizType);

		}
		BrandInfo brandInfo = (BrandInfo) prmtBrand.getValue();
		if (brandInfo != null)
			defaultRepairClassifyInfo = GAUtils.getDefaultRepairClassify(null,
					brandInfo, orgUnitInfo);
	...
	public void initF7Filter() {
		...
		kdtRWOItemSpEntry.addKDTSelectListener(new KDTSelectListener() {
			...
			public void tableSelectChanged(KDTSelectEvent kdtselectevent) {
				kdtRWOItemSpEntry.cellAtPosition(kdtselectevent
						.getSelectBlock().getBeginRow(), kdtselectevent
						.getSelectBlock().getBeginCol());
		...
		kdtRWOItemSpEntry.addKDTEditListener(new KDTEditAdapter() {
			...
			public void editStarting(KDTEditEvent kdteditevent) {
				...
				kdtRWOItemSpEntry.cellAtPosition(kdteditevent.getRowIndex(),
						kdteditevent.getColIndex());
			}
		...
		EventListenerList lstListenerRepairItem = kdtRWORepairItemEntry
				.getListenerList();
		if (lstListenerRepairItem != null) {
			Object[] listenerRepairItem = lstListenerRepairItem
					.getListeners(KDTEditListener.class);
			for (int i = 0; listenerRepairItem != null
					&& i < listenerRepairItem.length; i++) {
				String listenerNameRepairItem = listenerRepairItem[i]
						.toString();
				// 去掉RepairWOEditUI$添加的匿名监听
				if (listenerNameRepairItem
						.startsWith("com.kingdee.eas.auto4s.rsm.rs.client.RepairWOEditUI$"))
					kdtRWORepairItemEntry
							.removeKDTEditListener((KDTEditListener) listenerRepairItem[i]);
			}
		}
			...
			public void sort(int colIndex, int sortType) {
				if (pkgIndex == colIndex) {
					try {
						...
						sortColumns.add(new SortColumnInfo("repairPkg",
								SortTypeEnum.getEnum(sortType)));
						sortColumns.add(new SortColumnInfo("t", SortTypeEnum
								.getEnum(sortType)));
						sortColumns.add(new SortColumnInfo("seq", SortTypeEnum
								.getEnum(sortType)));

						KDTableComparatorUtils tblComparatorUtils = new KDTableComparatorUtils(
								kdtRWOItemSpEntry, sortColumns);
	...
	public void prmtCustomerAccount_Changed() throws Exception {
		...
		CustomerAccountInfo customerAccountInfo = (CustomerAccountInfo) prmtCustomerAccount.getData();
		if (customerAccountInfo == null) return;
		String typeCode = customerAccountInfo.getTypeCode();
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			IRow row = kdtRWOItemSpEntry.getRow(i);
			IEnum iType = (IEnum)row.getCell("i").getValue();
			if (PublicUtils.equals(IEnum.X, iType)) continue;
			...
			if (PublicUtils.equals(wInfo.getTypeCode(), typeCode)) {
				row.getCell("i").setValue(IEnum.I);
			} else {
				row.getCell("i").setValue(IEnum.H);
			}
			resetItemSpEditorLocked(row);
	...
	protected void kdtRWORepairItemEntry_editStartingNew(KDTEditEvent e) {
		...
		if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_ActualWorkTime))
			kdtRWORepairItemEntry_Starting(e);
		if (tb.getColumn(e.getColIndex()).getFieldName().equals("RepairItem"))
			repairItemNumberBefore = getRepairItemNumber(kdtRWORepairItemEntry
					.getCell(e.getRowIndex(), "RepairItem").getValue());
	...
	protected void kdtRWORepairItemEntry_editStoppedNew(KDTEditEvent e) {
		KDTable tb = (KDTable) e.getSource();
		isModify = true;
		if (null != kdtRWORepairItemEntry.getCell(e.getRowIndex(),
				"SettleObject").getValue()) {
			SettlementObjectEnum setObject = (SettlementObjectEnum) kdtRWORepairItemEntry
					.getCell(e.getRowIndex(), "SettleObject").getValue();
		...
		try {
			if (tb.getColumn(e.getColIndex()).getFieldName().equals(
					"RepairItem")
					|| tb.getColumn(e.getColIndex()).getFieldName().equals(
							"SettleObject")
					|| tb.getColumn(e.getColIndex()).getFieldName().equals(
							"PaymentClassify")) {
				if (tb.getColumn(e.getColIndex()).getFieldName().equals(
						"RepairItem")) {
					String rtNumberAfter = getRepairItemNumber(kdtRWORepairItemEntry
							.getCell(e.getRowIndex(), "RepairItem").getValue());
					if (null != repairItemNumberBefore
							&& (null == rtNumberAfter || null != rtNumberAfter
									&& !RsUtils.isStringEquals(
											repairItemNumberBefore,
											rtNumberAfter)))
						setIsHadVipDisCountRate(kdtRWORepairItemEntry, e
								.getRowIndex(), false);
					kdtRWORepairItemEntry.getCell(e.getRowIndex(),
							"PaymentClassify").setValue(paymentClassifyInfo);
					if (paymentClassifyInfo != null)
						kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								"SettleObject").setValue(
								paymentClassifyInfo.getSettObj());
					kdtRWOItemEntryPamentChanged(e.getRowIndex(), e
							.getColIndex());
				}
				if (e.getValue() != e.getOldValue()) {
					if ((kdtRWORepairItemEntry.getCell(e.getRowIndex(),
							"RepairPkg").getValue() == null || isBasePkg(
							kdtRWORepairItemEntry, e.getRowIndex()))
							&& kdtRWORepairItemEntry.getCell(e.getRowIndex(),
									"ServiceActivity").getValue() == null)
						getDisRateEntry(e.getRowIndex());
					if ((null == kdtRWORepairItemEntry.getCell(e.getRowIndex(),
							"SettleObject").getValue() || (SettlementObjectEnum) kdtRWORepairItemEntry
							.getCell(e.getRowIndex(), "SettleObject")
							.getValue() != SettlementObjectEnum.CLUB)
							&& (kdtRWORepairItemEntry.getCell(e.getRowIndex(),
									"RepairPkg").getValue() == null || isBasePkg(
									kdtRWORepairItemEntry, e.getRowIndex()))
							&& kdtRWORepairItemEntry.getCell(e.getRowIndex(),
									"ServiceActivity").getValue() == null) {
						BigDecimal workTimeAmount = UIRuleUtil
								.getBigDecimal(kdtRWORepairItemEntry.getCell(
										e.getRowIndex(),
										RepairWOEditUI.Cell_WorkTimeAmount)
										.getValue());
						BigDecimal discountRate = UIRuleUtil
								.getBigDecimal(kdtRWORepairItemEntry.getCell(
										e.getRowIndex(),
										RepairWOEditUI.Cell_DiscountRate)
										.getValue());
						BigDecimal discountAmount = workTimeAmount.multiply(
								discountRate).divide(BIGDEC100, 4, 6);
						kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								RepairWOEditUI.Cell_DiscountAmount).setValue(
								discountAmount);
						BigDecimal amount = UIRuleUtil
								.getBigDecimal(workTimeAmount);
						amount = amount
								.multiply(BIGDEC1.subtract(discountRate));
						kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								RepairWOEditUI.Cell_ARAmount).setValue(amount);
						kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								"ActualAmount").setValue(amount);
						if ((kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								"RepairPkg").getValue() == null || isBasePkg(
								kdtRWORepairItemEntry, e.getRowIndex()))
								&& kdtRWORepairItemEntry.getCell(
										e.getRowIndex(), "ServiceActivity")
										.getValue() == null) {
							kdtRWORepairItemEntry
									.getCell(e.getRowIndex(), "WorkTimePrice")
									.setValue(
											StandardPriceWarraPrice((SettlementObjectEnum) kdtRWORepairItemEntry
													.getCell(e.getRowIndex(),
															"SettleObject")
													.getValue()));
							kdtRWORepairItemEntry.getCell(e.getRowIndex(),
									"WorkTimeStdPrice").setValue(
									workTimeStdPrice);
							kdtWorkTimeEntrycount(e.getRowIndex());
						}
					}
				}
				if (kdtRWORepairItemEntry.getCell(e.getRowIndex(), "RepairPkg")
						.getValue() == null
						&& kdtRWORepairItemEntry.getCell(e.getRowIndex(),
								"ServiceActivity").getValue() == null)
					if (tb.getColumn(e.getColIndex()).getFieldName().equals(
						...
						if (!RsUtils.isEqualSettlementObjectEnum(oldSO, newSO))
							setIsHadVipDisCountRate(kdtRWORepairItemEntry, e
									.getRowIndex(), false);
					} else if (tb.getColumn(e.getColIndex()).getFieldName()
							.equals("PaymentClassify")) {
						...
						if (null == oldPC
								...
								|| null != oldPC
								&& null != newPC
								&& !((PaymentClassifyInfo) oldPC).getId()
										.toString().equals(
												((PaymentClassifyInfo) newPC)
														.getId().toString()))
							setIsHadVipDisCountRate(kdtRWORepairItemEntry, e
									.getRowIndex(), false);
					}
		...
		if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_DiscountRate)) {
			BigDecimal old_DiscountRate = UIRuleUtil
					.getBigDecimal(kdtRWORepairItemEntry.getCell(
							e.getRowIndex(), "OldDiscountRate").getValue());
			BigDecimal DiscountRate = UIRuleUtil.getBigDecimal(e.getValue());
			if (paramValue_RSM003.equals("1")
					&& old_DiscountRate.compareTo(DiscountRate) == -1
					&& !paramValue_RSM012) {
				MsgBox.showInfo((new StringBuilder()).append(" 折扣率只能<= ")
						.append(old_DiscountRate).toString());
				DiscountRate = old_DiscountRate;
				tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_DiscountRate)
						.setValue(old_DiscountRate);
			}
			...
			boolean isChange = isDisCountRateChanged(e.getOldValue(), e
					.getValue());
			setIsHadVipDisCountRate(kdtRWORepairItemEntry, e.getRowIndex(),
					isChange);
			BigDecimal WorkTimeAmount = UIRuleUtil.getBigDecimal(tb.getCell(
					e.getRowIndex(), RepairWOEditUI.Cell_WorkTimeAmount)
					.getValue());
			tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_DiscountAmount)
					.setValue(
							WorkTimeAmount.multiply(DiscountRate).divide(
									BIGDEC100, 4, 6));
			tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_ARAmount).setValue(
					WorkTimeAmount.subtract(UIRuleUtil.getBigDecimal(tb
							.getCell(e.getRowIndex(),
									RepairWOEditUI.Cell_DiscountAmount)
							.getValue())));
			tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_ActualAmount)
					.setValue(
							UIRuleUtil.getBigDecimal(tb.getCell(
									e.getRowIndex(),
									RepairWOEditUI.Cell_ARAmount).getValue()));
		} else if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_StdWorkTime)) {
			if (null == e.getValue()
					|| (new BigDecimal(e.getValue().toString()))
							.compareTo(BIGDEC0) == -1) {
				// MsgBox.showInfo(" 标准工时必须 >=0 ");
				// tb.getCell(e.getRowIndex(),
				// RepairWOEditUI.Cell_StdWorkTime).setValue(e.getOldValue());
			}
			dealAfterWorkTimeChanged(e.getRowIndex(),
					RepairWOEditUI.Cell_StdWorkTime, e);
		} else if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_AssignWorkTime))
			dealAfterWorkTimeChanged(e.getRowIndex(),
					RepairWOEditUI.Cell_AssignWorkTime, e);
		else if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_WorkGroup)) {
			if (null == e.getValue()) {
				tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_Person)
						.setValue(null);
				tb.getCell(e.getRowIndex(), "WagePrice").setValue(BIGDEC0);
				tb.getCell(e.getRowIndex(), "WorkTimeCost").setValue(BIGDEC0);
			}
		} else if (tb.getColumn(e.getColIndex()).getFieldName().equals(
				RepairWOEditUI.Cell_Person)) {
			if (null == e.getValue() && (null == s || s.equalsIgnoreCase("0"))) {
				tb.getCell(e.getRowIndex(), "WagePrice").setValue(BIGDEC0);
				tb.getCell(e.getRowIndex(), "WorkTimeCost").setValue(BIGDEC0);
			}
		} else if (RepairWOEditUI.Cell_DiscountAmount.equals(tb.getColumn(
				e.getColIndex()).getFieldName())) {
			IRow row = tb.getRow(e.getRowIndex());
			BigDecimal DiscountRate = UIRuleUtil.getBigDecimal(row.getCell(
					RepairWOEditUI.Cell_DiscountRate).getValue());
			BigDecimal old_DiscountRate = UIRuleUtil.getBigDecimal(row.getCell(
					"OldDiscountRate").getValue());
			BigDecimal DiscountAmount = UIRuleUtil.getBigDecimal(e.getValue());
			BigDecimal WorkTimeAmount = UIRuleUtil.getBigDecimal(tb.getCell(
					e.getRowIndex(), RepairWOEditUI.Cell_WorkTimeAmount)
					.getValue());
			if (WorkTimeAmount.compareTo(BIGDEC0) != 0) {
				DiscountRate = DiscountAmount.multiply(BIGDEC100).divide(
						WorkTimeAmount, 4, 6);
			} else {
				row.getCell(RepairWOEditUI.Cell_DiscountAmount).setValue(
						UIRuleUtil.getBigDecimal(e.getOldValue()));
				DiscountAmount = UIRuleUtil.getBigDecimal(e.getOldValue());
			}
			if (null != paramValue_RSM003 && paramValue_RSM003.equals("1")
					&& old_DiscountRate.compareTo(DiscountRate) == -1
					&& !paramValue_RSM012) {
				row.getCell(RepairWOEditUI.Cell_DiscountRate).setValue(
						old_DiscountRate);
				DiscountRate = old_DiscountRate;
				row.getCell(RepairWOEditUI.Cell_DiscountAmount).setValue(
						UIRuleUtil.getBigDecimal(WorkTimeAmount.multiply(
								DiscountRate).divide(BIGDEC100, 4, 6)));
				DiscountAmount = UIRuleUtil.getBigDecimal(row.getCell(
						RepairWOEditUI.Cell_DiscountAmount).getValue());
				MsgBox.showInfo((new StringBuilder()).append(" 折扣率只能<= ")
						.append(old_DiscountRate).toString());
			}
			if (DiscountRate.compareTo(BIGDEC100) > 0 && !paramValue_RSM012) {
			...
			row.getCell(RepairWOEditUI.Cell_DiscountRate)
					.setValue(DiscountRate);
			row.getCell(RepairWOEditUI.Cell_DiscountAmount).setValue(
					DiscountAmount);
			row.getCell(RepairWOEditUI.Cell_ARAmount).setValue(
					WorkTimeAmount.subtract(DiscountAmount));
			row.getCell(RepairWOEditUI.Cell_ActualAmount).setValue(
					UIRuleUtil.getBigDecimal(row.getCell(
							RepairWOEditUI.Cell_ARAmount).getValue()));
		} else if ("RepairClassify".equals(tb.getColumn(e.getColIndex())
				.getFieldName())) {
			if (isChangeDiscountRateForRepairClassify(tb, e.getRowIndex())) {
				Object oldValue = e.getOldValue();
				Object newValue = e.getValue();
				if (oldValue == null && null != newValue || null == newValue
						&& null != oldValue || oldValue != null
						&& null != newValue && !oldValue.equals(newValue)) {
					isModify = true;
					IRow row = tb.getRow(e.getRowIndex());
					row.getCell(RepairWOEditUI.Cell_DiscountRate).setValue(
							BIGDEC0);
		...
		} else if (RepairWOEditUI.Cell_ARAmount.equals(tb.getColumn(
				e.getColIndex()).getFieldName())
				&& paramValue_RSM012) {
			...
			BigDecimal workTimeAmount = UIRuleUtil.getBigDecimal(tb.getCell(
					...
					.getValue());
			tb.getCell(e.getRowIndex(), RepairWOEditUI.Cell_DiscountAmount)
					.setValue(workTimeAmount.subtract(araAmount));
			if (workTimeAmount.compareTo(BIGDEC0) != 0)
				tb
						.getCell(e.getRowIndex(),
								RepairWOEditUI.Cell_DiscountRate)
	...
	protected void kdtRWOSparepartEntry_editStopped(KDTEditEvent e) {
		...
		if (null == row.getCell("Material").getValue()) {
			...
			row.getCell(Cell_AssistProperty).getStyleAttributes()
		...
		MaterialInfo materialInfo = (MaterialInfo) row.getCell("Material")
				.getValue();
		BigDecimal IssueQty = UIRuleUtil.getBigDecimal(row.getCell(
				Cell_IssueQty).getValue());
		BigDecimal newValue = null;
		IColumn Unit = kdtRWOSparepartEntry.getColumn(Cell_Unit);
		if (null != materialInfo)
			RsQueryF7Utils.setMaterialUnitQuery(Unit, null, materialInfo);
		MeasureUnitInfo unitInfo = row.getCell(Cell_Unit).getValue() != null ? (MeasureUnitInfo) row
				.getCell(Cell_Unit).getValue()
				: null;
		MeasureUnitInfo baseUnitInfo = row.getCell(Cell_BaseUnit).getValue() != null ? (MeasureUnitInfo) row
				.getCell(Cell_BaseUnit).getValue()
				: null;
		BigDecimal TaxRate = UIRuleUtil.getBigDecimal(row.getCell(Cell_TaxRate)
				.getValue());
		BigDecimal DiscountRate = UIRuleUtil.getBigDecimal(row.getCell(
				Cell_DiscountRate).getValue());
		BigDecimal DiscountAmount = UIRuleUtil.getBigDecimal(row.getCell(
				Cell_DiscountAmount).getValue());
		BigDecimal TaxPrice = UIRuleUtil.getBigDecimal(row.getCell(
				Cell_TaxPrice).getValue());
		BigDecimal qty = UIRuleUtil.getBigDecimal(row.getCell(Cell_Qty)
				.getValue());
		BigDecimal temp = BIGDEC1;
		if (Cell_Qty.equals(curCellKey)) {
			newValue = e.getValue() != null ? UIRuleUtil.getBigDecimal(e
					.getValue()) : BIGDEC0;
			if (null != row.getCell(Cell_ID).getValue()) {
				...
				try {
					if (iRepairWORWOSparepartEntry.exists(new ObjectUuidPK(
							rowID)))
						info = iRepairWORWOSparepartEntry
								.getRepairWORWOSparepartEntryInfo(new ObjectUuidPK(
										rowID));
					if (null != info)
			...
			TaxPrice = UIRuleUtil.getBigDecimal(row.getCell(Cell_TaxPrice)
					.getValue());
			row.getCell(Cell_NoTaxPrice).setValue(
					TaxPrice.multiply(BIGDEC100).divide(BIGDEC100.add(TaxRate),
							4, 6));
			row.getCell(Cell_NoIssueQty).setValue(newValue.subtract(IssueQty));
			try {
				row.getCell(Cell_BaseQty).setValue(
						CommonUtils.getBaseUnitQty(materialInfo, baseUnitInfo,
								unitInfo, newValue));
			} catch (EASBizException e1) {
				handUIException(e1);
			} catch (BOSException e1) {
			...
			row.getCell(Cell_DiscountAmount).setValue(
					temp.multiply(DiscountRate).divide(BIGDEC100, 4, 6));
			row.getCell(Cell_ARAmount).setValue(
					temp.subtract(UIRuleUtil.getBigDecimal(row.getCell(
							Cell_DiscountAmount).getValue())));
			row.getCell(Cell_ActualAmount).setValue(
					UIRuleUtil.getBigDecimal(row.getCell(Cell_ARAmount)
							.getValue()));
			row.getCell(Cell_Tax).setValue(
					UIRuleUtil.getBigDecimal(
							row.getCell(Cell_ARAmount).getValue()).multiply(
							TaxRate).divide(BIGDEC100.add(TaxRate), 4, 6));
			temp = UIRuleUtil.getBigDecimal(row.getCell(Cell_NoTaxPrice)
					.getValue());
			row.getCell(Cell_NoTaxAmount).setValue(newValue.multiply(temp));
		} else if (Cell_Unit.equals(curCellKey)) {
			unitInfo = e.getValue() != null ? (MeasureUnitInfo) e.getValue()
					: null;
			if (null != unitInfo)
				try {
					row.getCell(Cell_BaseQty).setValue(
							CommonUtils.getBaseUnitQty(materialInfo,
									baseUnitInfo, unitInfo, qty));
				} catch (EASBizException e1) {
					handUIException(e1);
				} catch (BOSException e1) {
					handUIException(e1);
			...
			row.getCell(Cell_NoTaxPrice).setValue(
					TaxPrice.multiply(BIGDEC100).divide(BIGDEC100.add(TaxRate),
							4, 6));
			temp = UIRuleUtil.getBigDecimal(row.getCell(Cell_ARAmount)
					.getValue());
			row.getCell(Cell_Tax)
					.setValue(
							temp.multiply(TaxRate).divide(
									BIGDEC100.add(TaxRate), 4, 6));
			temp = UIRuleUtil.getBigDecimal(row.getCell(Cell_NoTaxPrice)
					.getValue());
			row.getCell(Cell_NoTaxAmount).setValue(qty.multiply(temp));
		} else if (Cell_DiscountRate.equals(curCellKey)) {
			BigDecimal old_DiscountRate = UIRuleUtil
					.getBigDecimal(kdtRWOSparepartEntry.getCell(
							row.getRowIndex(), "OldDiscountRate").getValue());
			...
			temp = UIRuleUtil.getBigDecimal(row.getCell(Cell_TaxAmount)
					.getValue());
			row.getCell(Cell_DiscountAmount).setValue(
					temp.multiply(DiscountRate).divide(BIGDEC100, 4, 6));
			temp = UIRuleUtil.getBigDecimal(row.getCell(Cell_DiscountAmount)
					.getValue());
			row.getCell(Cell_ARAmount).setValue(
					UIRuleUtil.getBigDecimal(
							row.getCell(Cell_TaxAmount).getValue()).subtract(
							temp));
			row.getCell(Cell_ActualAmount).setValue(
					UIRuleUtil.getBigDecimal(row.getCell(Cell_ARAmount)
							.getValue()));
			...
			if (temp.compareTo(BIGDEC0) != 0) {
				DiscountRate = DiscountAmount.multiply(BIGDEC100).divide(temp,
						4, 6);
			} else {
				row.getCell(Cell_DiscountAmount).setValue(
						UIRuleUtil.getBigDecimal(e.getOldValue()));
			...
			row.getCell(Cell_DiscountRate).setValue(DiscountRate);
			BigDecimal noTaxPrice = newTaxPrice.multiply(BIGDEC100).divide(
					BIGDEC100.add(TaxRate), 4, 6);
			...
			row.getCell(Cell_DiscountAmount).setValue(
					newTaxPrice.multiply(qty).multiply(DiscountRate).divide(
							BIGDEC100, 4, 6));
			row.getCell(Cell_ARAmount).setValue(
					newTaxPrice.multiply(qty).subtract(
							UIRuleUtil.getBigDecimal(row.getCell(
									Cell_DiscountAmount).getValue())));
			row.getCell(Cell_ActualAmount).setValue(
					UIRuleUtil.getBigDecimal(row.getCell(Cell_ARAmount)
							.getValue()));
			BigDecimal tax = UIRuleUtil.getBigDecimal(
					row.getCell(Cell_ARAmount).getValue()).multiply(TaxRate)
					.divide(BIGDEC100.add(TaxRate), 4, 6);
			row.getCell(Cell_Tax).setValue(tax);
			isModify = true;
		} else if ("RepairClassify".equals(curCellKey)) {
			if (isChangeDiscountRateForRepairClassify(kdtRWOSparepartEntry,
					curRow)) {
				...
				if (null != newRValue
						...
						|| oldRValue != null
						&& null != newRValue
						&& !((RepairClassifyInfo) oldRValue).getId().toString()
								.equals(
										((RepairClassifyInfo) newRValue)
												.getId().toString())) {
					isModify = true;
					row.getCell(Cell_DiscountRate).setValue(BIGDEC0);
			...
			if (temp.compareTo(BIGDEC0) != 0) {
				DiscountRate = disCountAmountbyAr.multiply(BIGDEC100).divide(
						temp, 4, 6);
			} else {
				row.getCell(Cell_ARAmount).setValue(
						UIRuleUtil.getBigDecimal(e.getOldValue()));
	...
	private void appendItemSpFoot() {
		...
		// footManager.getMergeManager().setMergeMode(KDTMergeManager.
		// FREE_COLUMN_MERGE);
		if (footRow == null)
		...
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			...
			if (!PublicUtils.equals(IEnum.I, iType))
				continue;
			try {
				BigDecimal amount = PublicUtils.getBigDecimal(row.getCell(
						"amount").getValue(), 2);
				totalAmount = totalAmount.add(amount);
				BigDecimal taxAmount = PublicUtils.getBigDecimal(row.getCell(
						"taxAmount").getValue(), 2);
		...
		footRow.getCell(tIndex).getStyleAttributes().setHorizontalAlign(
				HorizontalAlignment.RIGHT);

		footRow.getCell(itemspNameIndex).setValue(totalAmount);
		footRow.getCell(itemspNameIndex).getStyleAttributes().setNumberFormat(
				"%r{#,##0.00}f");
		footRow.getCell(itemspNameIndex).getStyleAttributes()
				.setHorizontalAlign(HorizontalAlignment.LEFT);

		footRow.getCell(repairPkgIndex).setValue("增值税");
		footRow.getCell(repairPkgIndex).getStyleAttributes()
				.setHorizontalAlign(HorizontalAlignment.RIGHT);
		footRow.getCell(qtyIndex).setValue(totalTax);
		footRow.getCell(qtyIndex).getStyleAttributes().setNumberFormat(
				"%r{#,##0.00}f");
		footRow.getCell(qtyIndex).getStyleAttributes().setHorizontalAlign(
				HorizontalAlignment.LEFT);

		footRow.getCell(priceIndex).setValue("含税合计");
		footRow.getCell(priceIndex).getStyleAttributes().setHorizontalAlign(
				HorizontalAlignment.RIGHT);

		footRow.getCell(discountRateIndex).setValue(totalTaxAmount);
		footRow.getCell(discountRateIndex).getStyleAttributes()
	...
	private void valueChangedForItemSpEntry(Object oldValue, KDTEditEvent e)
			throws Exception {
		...
		if (colIndex == wIndex) {
			...
			if (colIndex == amountIndex) {
				amount = PublicUtils.getBigDecimal(kdtRWOItemSpEntry.getRow(
						rowIndex).getCell(amountIndex).getValue());
				if (PublicUtils.equals(price, BIGDEC0)
						|| PublicUtils.equals(qty, BIGDEC0))
					discountRate = BIGDEC0;
				else
					discountRate = BIGDEC100.subtract(amount.divide(
							price.multiply(qty), 10, BigDecimal.ROUND_HALF_UP)
							.multiply(BIGDEC100));
				taxAmount = amount.multiply(BIGDEC1.add(taxRate.divide(
						BIGDEC100, 10, BigDecimal.ROUND_HALF_UP)));

			} else if (colIndex == taxAmountIndex) {
				taxAmount = PublicUtils.getBigDecimal(kdtRWOItemSpEntry.getRow(
						rowIndex).getCell(taxAmountIndex).getValue());
				if (PublicUtils.equals(taxPrice, BIGDEC0)
						|| PublicUtils.equals(qty, BIGDEC0))
					discountRate = BIGDEC0;
				else
					discountRate = BIGDEC100.subtract(taxAmount.divide(
							taxPrice.multiply(qty), 10,
							BigDecimal.ROUND_HALF_UP).multiply(BIGDEC100));
				amount = taxAmount.divide(BIGDEC1.add(taxRate.divide(BIGDEC100,
						10, BigDecimal.ROUND_HALF_UP)), 10,
						BigDecimal.ROUND_HALF_UP);
			}
			BigDecimal initFactPrice = PublicUtils.getBigDecimal(row.getCell(
					initFactPriceIndex).getValue());
			BigDecimal factPrice = taxPrice.multiply(BIGDEC1
					.subtract(discountRate.divide(BIGDEC100, 10,
							BigDecimal.ROUND_HALF_UP)));
			...
			if (PublicUtils.equals(TEnum.L, tType)
				...
				MsgBoxEx.showInfo(String.format("折扣不能高于折扣权限[%s]！",
						maxRepairDiscountRate.setScale(2)));
				kdtRWOItemSpEntry.getRow(rowIndex).getCell(colIndex).setValue(
						oldValue);
				return;
			} else if (PublicUtils.equals(TEnum.P, tType)
					&& factPrice.compareTo(initFactPrice) != 0
			...
			kdtRWOItemSpEntry.getRow(rowIndex).getCell(amountIndex).setValue(
					amount);
			kdtRWOItemSpEntry.getRow(rowIndex).getCell(taxAmountIndex)
					.setValue(taxAmount);
			kdtRWOItemSpEntry.getRow(rowIndex).getCell(discountRateIndex)
					.setValue(discountRate);
	...
	private BigDecimal getMaterialCostPrice(String stroageUnitId,
			MaterialInfo materialInfo) throws Exception {
		if (stroageUnitId == null || materialInfo == null)
			return BIGDEC0;

		BigDecimal costPrice = null;
		String sql = String
				.format(
						"select CFCostPrice from  T_BD_MaterialInventory where FMaterialID='%s' and FOrgUnit='%s'",
		...
		sql = String
				.format(
						"select isnull(FPrice,0) from T_BD_MaterialPurchasing where FMaterialID='%s' and FOrgUnit='%s'",
						materialInfo.getString("id"), stroageUnitId);
		rs = DBUtils.executeQuery(null, sql);
		if (rs != null && rs.next()) {
			costPrice = rs.getBigDecimal(1);
			if (costPrice != null)
				return costPrice.multiply(new BigDecimal(1.17));
		}
	...
	public void resetItemSpEditorLocked(IRow row) {
		...
		if (PublicUtils.equals(IEnum.H, cellI.getValue())) {
			...
			for (int i = 0; i < kdtRWOItemSpEntry.getColumnCount(); i++) {
				...
				else row.getCell(i).getStyleAttributes().setLocked(true);
				//else row.getCell(i).getStyleAttributes().setLocked(true);
			}
		...
		if ("L".equals(cellT.getValue().toString())) { // 项目
			...
			RepairItemInfo repairItemInfo = (RepairItemInfo) cellItem
					.getValue();
			if (repairItemInfo != null
					&& !repairItemInfo.getNumber().startsWith("FDJQ")) {
				row.getCell("taxPrice").getStyleAttributes().setLocked(
						!hasPermission_OprtRepairTaxPrice);
				row.getCell("taxAmount").getStyleAttributes().setLocked(
						!hasPermission_OprtRepairTaxPrice);
				row.getCell("price").getStyleAttributes().setLocked(
						!hasPermission_OprtRepairTaxPrice);
			...
			row.getCell("itemspName").getStyleAttributes().setLocked(
		...
		if (!hasPermission_OprtRetailLine
				&& PublicUtils.equals("P", cellT.getValue().toString())) {
			row.getStyleAttributes().setLocked(true);

		} else if (!hasPermission_OprtRepairLine
				&& PublicUtils.equals("L", cellT.getValue().toString())) {
			row.getStyleAttributes().setLocked(true);
		}
		...
		try {
			...
			if (issueQty.compareTo(BIGDEC0) != 0
					&& PublicUtils.equals(IEnum.X, iType)) { // 已有出库
				row.getStyleAttributes().setLocked(true);
			}
			...
			if (isAPSettle) { // 应付结算
				...
				if (PublicUtils.equals(RepairWayEnum.DEFAULT, row.getCell("repairWay").getValue()))
					row.getCell("supplier").getStyleAttributes().setLocked(true);
				else
		...
		if (vehicleInfo != null) {
			...
			if (rs != null && rs.next()) {
				...
				if (rs1 != null && rs1.next()) {
					...
					if (PublicUtils.equals("无名称", name))
						name = "现结客户";
					String tel = StringUtils.cnulls(rs1.getString("FPhone"));
					if (PublicUtils.equals("无号码", tel))
						tel = "";
	...
	private void getRepairWoRemarkHostory() throws Exception {
		...
		IVehicleRepairRemark repairRemark = VehicleRepairRemarkFactory
				.getRemoteInstance();
		...
		String sql = String.format(
				"where parent.id='%s' %s order by createTime desc", vehicleInfo
						.getString("id"), rwSql);
		VehicleRepairRemarkCollection repairRemarkCol = repairRemark
				.getVehicleRepairRemarkCollection(sql);
		txtRemarkList.setText("");
	...
	public void initUIContentLayout() {
		...
		kDTRWOPane.insertTab(resHelper.getString("kDPRwoItemSp.constraints"),
				null, kDPRwoItemSp, resHelper
						.getString("kDPRwoItemSp.constraints"), 0);
		...
		contRepairSender.setBounds(rRWOMan.x, rRWOMan.y, rVin.width
				- rBtn.width + 5, rRWOMan.height);
		kDPBizInfo.add(contRepairSender, new KDLayout.Constraints(rRWOMan.x,
				rRWOMan.y, rVin.width - rBtn.width + 5, rRWOMan.height,
				KDLayout.Constraints.ANCHOR_TOP
						| KDLayout.Constraints.ANCHOR_LEFT_SCALE
						| KDLayout.Constraints.ANCHOR_RIGHT_SCALE));
		btnShowRepairSender.setBounds(rVin.x + rVin.width - rBtn.width + 5,
				rBtn.y, rBtn.width, rBtn.height);
		kDPBizInfo.add(btnShowRepairSender, new KDLayout.Constraints(rVin.x
				+ rVin.width - rBtn.width + 5, rBtn.y, rBtn.width, rBtn.height,
				KDLayout.Constraints.ANCHOR_TOP
						| KDLayout.Constraints.ANCHOR_LEFT_SCALE));
	...
	protected IObjectValue createNewDetailData(KDTable table) {
		if (kdtRWOItemSpEntry.getName().equals(table.getName())) {
			...
			itemSpEntryInfo.setRepairWay((RepairWayEnum) repairWay
					.getSelectedItem());
			// itemSpEntryInfo.setSupplier((SupplierInfo)
			// prmtSupplier.getValue());
	...
	private HashMap<String, BigDecimal> getDiscountRate() throws Exception {
		...
		if (retailDiscountRate == null
				...
				|| repairDiscountRate == null
				|| repairDiscountRate.compareTo(BIGDEC0) == 0) {
			CustomerInfo atsCustomerInfo = (CustomerInfo) prmtCustomer
					.getValue();
			Date dateComeTime = (Date) pkComeTime.getValue();

			if (atsCustomerInfo == null || dateComeTime == null)
		...
		hashDiscountRate.put(DiscountDate_Retail, PublicUtils
				.getBigDecimal(retailDiscountRate));
		hashDiscountRate.put(DiscountDate_Repair, PublicUtils
				.getBigDecimal(repairDiscountRate));
		return hashDiscountRate;
	}
	...
	public void loadFields() {
		...
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			resetItemSpEditorLocked(kdtRWOItemSpEntry.getRow(i));
			IRow row = kdtRWOItemSpEntry.getRow(i);
			RepairItemInfo repairItemInfo = (RepairItemInfo) row.getCell(
					"repairItem").getValue();
			String itemspNum = (String) row.getCell("itemspNum").getValue();
			if (repairItemInfo != null
					&& !PublicUtils.isEmpty(repairItemInfo.getNumber())
					&& !PublicUtils.equals(itemspNum, repairItemInfo
							.getNumber())) {
	...
	private void setCmbT_Permission() {
		if (hasPermission_OprtRepairLine && !hasPermission_OprtRetailLine) { // 只有维修行权限
			...
			labRepairItem.setVisible(true);
				...
				&& hasPermission_OprtRetailLine) { // 只有配件行权限
			cmbT.setEnabled(false);
			cmbT.setSelectedItem(TEnum.P);
			labMaterial.setVisible(true);
		} else if (hasPermission_OprtRepairLine && hasPermission_OprtRetailLine) { // 有配件
			// 、
			// 维修行权限
	...
	public void actionAddLine_actionPerformed(ActionEvent e) throws Exception {
		...
		if (kDTRWOPane.getSelectedIndex() == 5
				&& kdtRWOAttachmentItemEntry != null) {
	...
	private boolean checkRemoveLine() throws Exception {
		if (kdtRWOItemSpEntry.getSelectManager().size() == 0)
			return true;
		...
		/** 将分录ID关联维修项目ID的分录行号，缓存 */
		HashMap<String, Vector<Integer>> hashItemSpEntryRowIndexCache = new HashMap<String, Vector<Integer>>();
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			IRow row = kdtRWOItemSpEntry.getRow(i);
			RepairWORWOItemSpEntryInfo rwoItemSpEntryInfo = (RepairWORWOItemSpEntryInfo) row
					.getUserObject();
			String id = rwoItemSpEntryInfo.getString("id");
			String relateItemEntryId = rwoItemSpEntryInfo
					.getRelateItemEntryId();
			String key = id;
			if (!PublicUtils.isEmpty(relateItemEntryId))
				key = relateItemEntryId;
			Vector<Integer> value = hashItemSpEntryRowIndexCache.get(key);
			if (value == null)
				value = new Vector<Integer>();
			value.add(i);
			hashItemSpEntryRowIndexCache.put(key, value);
		}
		...
		for (int i = 0; i < lstItemSpEntry.size(); i++) {
			...
			for (int j = sb.getBeginRow(); j <= sb.getEndRow(); j++) {
				hashSelRow.put(j, j);
				// 检查是否关联维修配件行，若是，将所有行号并入选中行
				IRow row = kdtRWOItemSpEntry.getRow(j);
				RepairWORWOItemSpEntryInfo rwoItemSpEntryInfo = (RepairWORWOItemSpEntryInfo) row
						.getUserObject();
				String id = rwoItemSpEntryInfo.getString("id");
				String relateItemEntryId = rwoItemSpEntryInfo
				...
				if (!PublicUtils.isEmpty(relateItemEntryId))
					key = relateItemEntryId;
				Vector<Integer> value = hashItemSpEntryRowIndexCache.get(key);
				for (int k = 0; value != null && k < value.size(); k++) {
					hashSelRow.put(value.get(k), value.get(k));
					KDTSelectBlock newSb = new KDTSelectBlock(value.get(k), 1,
							value.get(k), 0);
					kdtRWOItemSpEntry.getSelectManager().add(newSb);
				}
			}
		}
		...
		for (int i = 0; iSelRow != null && i < iSelRow.length; i++) {
			...
			RepairWORWOItemSpEntryInfo rwoItemSpEntryInfo = (RepairWORWOItemSpEntryInfo) rowItemSpEntry
					.getUserObject();
			...
			if (TEnum.L.equals(tType)) { // 维修项目
				...
				int removeRowIndex = getRowIndexByItemSpEntryId(
						kdtRWORepairItemEntry, itemSpEntryId);
				if (removeRowIndex > -1) {
					IRow row = kdtRWORepairItemEntry.getRow(removeRowIndex);
					boolean currentCanRemoveLine = canRemoveLineForRepairItemEntry(
							row, errMsg);
					if (isCanRemoveLine)
						isCanRemoveLine = currentCanRemoveLine;
				}

			} else if (TEnum.P.equals(tType)) { // 配件
				// 纯配件行
				if (!hasPermission_OprtRetailLine
						&& PublicUtils.isEmpty(relateItemEntryId)) {
				...
				if (!hasPermission_OprtRepairLine
						&& !PublicUtils.isEmpty(relateItemEntryId)) {
					MsgBoxEx.showInfo("无操作维修项目关联配件数据行权限！");
					return false;
				}
				...
				int removeRowIndex = getRowIndexByItemSpEntryId(
						kdtRWOSparepartEntry, itemSpEntryId);
				if (removeRowIndex > -1) {
					IRow row = kdtRWOSparepartEntry.getRow(removeRowIndex);
					boolean currentCanRemoveLine = canRemoveLineForSparepartEntry(
							row, errMsg);
					if (isCanRemoveLine)
						isCanRemoveLine = currentCanRemoveLine;
				}
		...
		if (!isCanRemoveLine) {
			MsgBoxEx.showDetailAndOK(this, "删除行失败，请查看明细", errMsg.toString(),
					MsgBox.OK);
			return false;
		}
		for (int i = 0; iSelRow != null && i < iSelRow.length; i++) {
	...
	private boolean canRemoveLineForRepairItemEntry(IRow row,
			StringBuilder errMsg) throws Exception {
		if (oprtState.equals("ADDNEW")
			...
			if (row.getCell("IsAppend").getValue() == null
					...
					&& !row.getCell(CeLL_ItemStatus).getValue().equals(
							RepairItemStatusEnum.SENDING)) {
				if (!row.getCell(CeLL_ItemStatus).getValue().equals(
						RepairItemStatusEnum.REPAIRING)
						&& !row.getCell(CeLL_ItemStatus).getValue().equals(
								RepairItemStatusEnum.FINISHED)
						&& !row.getCell(CeLL_ItemStatus).getValue().equals(
								RepairItemStatusEnum.INSPECTED)) {
				} else {
					RepairItemInfo repairItemInfo = (RepairItemInfo) row
							.getCell("RepairItem").getValue();
					String msg = "已派工的维修项目，不能进行删除操作！";
					if (repairItemInfo != null)
						msg = String.format("已派工[%s(%s)]的维修项目，不能进行删除操作！",
								repairItemInfo.getName(), repairItemInfo
										.getNumber());
	...
	private boolean canRemoveLineForSparepartEntry(IRow row,
			StringBuilder errMsg) throws Exception {
		if (oprtState.equals("ADDNEW")
				|| RepairBillStatusEnum.SAVE.equals(editData.getStatus())
				|| RepairBillStatusEnum.SENDING.equals(editData.getStatus())
			...
			if (!oprtState.equals("ADDNEW")) {
				if (null != row.getCell("ID").getValue()) {
					if (null == row.getCell("Material").getValue()
							...
							&& ((Object[]) (Object[]) row.getCell("Material")
									.getValue()).length == 0)
						removeLine(kdtRWOSparepartEntry);
					MaterialInfo materialInfo = (MaterialInfo) row.getCell(
							"Material").getValue();
					String rowID = UIRuleUtil.getString(row.getCell("ID")
							.getValue());
					if (null != hadIssueMaterialSet
							&& hadIssueMaterialSet.contains(rowID)) {
						String msg = String.format(
								"配件[%s(%s)]己经生成过出库(或退货)单,不能删除！", materialInfo
										.getName(), materialInfo.getNumber());
						errMsg.append(msg).append(CR);
						return false;
					}
				}
				if (row.getCell(Cell_IssueQty).getValue() != null) {
					if (UIRuleUtil.getBigDecimal(
							row.getCell(Cell_IssueQty).getValue()).compareTo(
							BigDecimal.valueOf(0L)) == 1) {
						MaterialInfo materialInfo = (MaterialInfo) row.getCell(
								"Material").getValue();
						String msg = String.format("配件[%s(%s)]已出库，不能进行删除操作！",
								materialInfo.getName(), materialInfo
										.getNumber());
						errMsg.append(msg).append(CR);
						return false;
					} else if (row.getCell("IsAppend").getValue().equals(
							Boolean.FALSE)
							&& (RepairBillStatusEnum.REPAIRING.equals(editData
									.getStatus())
									|| RepairBillStatusEnum.BREAK
											.equals(editData.getStatus())
									|| RepairBillStatusEnum.FINISH
											.equals(editData.getStatus()) || RepairBillStatusEnum.INSPECT
									.equals(editData.getStatus()))) {
						if (UIRuleUtil.getBigDecimal(
								row.getCell(Cell_IssueQty).getValue())
								.compareTo(BigDecimal.valueOf(0L)) == 1) {
							MaterialInfo materialInfo = (MaterialInfo) row
									.getCell("Material").getValue();
							String msg = String.format(
									"配件[%s(%s)]已出库，不能进行删除操作！", materialInfo
											.getName(), materialInfo
											.getNumber());
							errMsg.append(msg).append(CR);
							return false;
						}
						...
						&& row.getCell("IsAppend").getValue().equals(
								Boolean.FALSE)
						&& (RepairBillStatusEnum.REPAIRING.equals(editData
								.getStatus())
								|| RepairBillStatusEnum.BREAK.equals(editData
										.getStatus())
								|| RepairBillStatusEnum.FINISH.equals(editData
										.getStatus())
								|| RepairBillStatusEnum.WASH.equals(editData
										.getStatus()) || RepairBillStatusEnum.INSPECT
								.equals(editData.getStatus()))) {
					row.getCell("IsDelete").setValue(Boolean.TRUE);
					isDisplayDeleteRows(kdtRWOSparepartEntry, row.getRowIndex());
				} else if (UIRuleUtil.getBigDecimal(
						row.getCell(Cell_IssueQty).getValue()).compareTo(
						BigDecimal.valueOf(0L)) == 1) {
					MaterialInfo materialInfo = (MaterialInfo) row.getCell(
							"Material").getValue();
					String msg = String.format("配件[%s(%s)]已出库，不能进行删除操作！",
							materialInfo.getName(), materialInfo.getNumber());
					errMsg.append(msg).append(CR);
					return false;
				}
			}
	...
	private int getRowIndexByItemSpEntryId(KDTable tbl, String itemSpEntryId) {
		for (int i = 0; i < tbl.getRowCount(); i++) {
			...
			String rowItemSpEntryId = row.getCell("itemSpEntryId").getValue()
					.toString();
			if (itemSpEntryId.equals(rowItemSpEntryId))
				return i;
		}
		return -1;
	...
	public void actionRemoveLine_actionPerformed(ActionEvent e)
		...
		if (kDTRWOPane.getSelectedIndex() == 1) {
			if (kdtRWORepairItemEntry != null)
				if (oprtState.equals("ADDNEW")
						|| RepairBillStatusEnum.SAVE.equals(editData
								.getStatus())
						|| RepairBillStatusEnum.SENDING.equals(editData
								.getStatus())
						|| RepairBillStatusEnum.ABOLISH.equals(editData
								.getStatus())) {
					...
					if (getSelectedRow(kdtRWORepairItemEntry).getCell(
							"IsAppend").getValue() == null
							|| getSelectedRow(kdtRWORepairItemEntry).getCell(
									"IsAppend").getValue()
									.equals(Boolean.FALSE)
							|| getSelectedRow(kdtRWORepairItemEntry).getCell(
									CeLL_ItemStatus).getValue() != null
							&& !getSelectedRow(kdtRWORepairItemEntry).getCell(
									CeLL_ItemStatus).getValue().equals(
									RepairItemStatusEnum.SENDING)) {
						if (!getSelectedRow(kdtRWORepairItemEntry).getCell(
								CeLL_ItemStatus).getValue().equals(
								RepairItemStatusEnum.REPAIRING)
								&& !getSelectedRow(kdtRWORepairItemEntry)
										.getCell(CeLL_ItemStatus).getValue()
										.equals(RepairItemStatusEnum.FINISHED)
								&& !getSelectedRow(kdtRWORepairItemEntry)
										.getCell(CeLL_ItemStatus).getValue()
										.equals(RepairItemStatusEnum.INSPECTED)) {
							getSelectedRow(kdtRWORepairItemEntry).getCell(
									"IsDelete").setValue(Boolean.TRUE);
							isDisplayDeleteRows(kdtRWORepairItemEntry,
									getSelectedRow(kdtRWORepairItemEntry)
											.getRowIndex());
							getSelectedRow(kdtRWORepairItemEntry)
									.getStyleAttributes().setLocked(true);
							kdtRWORepairItemEntry_detailPanel
									.getRemoveLinesButton().setEnabled(false);
						} else {
	...
	private void registePrmtVehicleF7_Vin() {
		...
		// prmtVin.setQueryInfo(
		// "com.kingdee.eas.auto4s.rsm.rs.app.Copy_VehicleF7Query");
	...
	private void reRegistePrmtVehicleF7() {
		prmtVehicle.getQueryAgent().setHasCUDefaultFilter(false);
		// prmtVin.setQueryInfo(
		// "com.kingdee.eas.auto4s.rsm.rs.app.Copy_VehicleF7Query");
		prmtVehicle.setEditFormat("$plateNum$");
		prmtVehicle.setDisplayFormat("$plateNum$");
		prmtVehicle.setCommitFormat("$plateNum$");
		// VehicleRSMPromptBox selector = new VehicleRSMPromptBox(this);
		prmtVehicle.setEditable(true);
		// prmtVin.setSelector(selector);
		prmtVehicle.setEnabledMultiSelection(false);

		SelectorListener[] sclistener = prmtVehicle.getSelectorListeners();
		for (int i = 0; sclistener != null && i < sclistener.length; i++)
			prmtVehicle.removeSelectorListener(sclistener[i]);
		prmtVehicle.setSelector(null);
		prmtVehicle.addSelectorListener(new SelectorListener() {
			VehicleRSMPromptBoxEx selector = new VehicleRSMPromptBoxEx();

			public void willShow(SelectorEvent selectorevent) {
				prmtVehicle.setSelector(selector);
				selector.ctx.put("selIndex", 1);
				selector.ctx.put("value", prmtVehicle.getText().trim());
			}

		});
		/*
		 * prmtVin.addDataChangeListener(new DataChangeListener() { public void
		 * dataChanged(DataChangeEvent e) { boolean isChange = true; isChange =
		 * MakeControl.isF7ValueChanged(e); if(!isChange) return;
		 * prmtVehicle.setValue(prmtVin.getValue()); }
		 * 
		 * });
		 */
		KeyListener[] keyListener = prmtVehicle.getEditor().getKeyListeners();
		for (int i = 0; keyListener != null && i < keyListener.length; i++)
			prmtVehicle.getEditor().removeKeyListener(keyListener[i]);
		prmtVehicle.getEditor().addKeyListener(new KeyAdapter() {
			VehicleRSMPromptBoxEx selector = new VehicleRSMPromptBoxEx();
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == 10) {
					...
					if (selector.getData() != null) {
						prmtVehicle
								.setValue(((Object[]) selector.getData())[0]);
					}
	...
	private void registePrmtRepairItemF7() {
		...
		kdtRWORepairItemEntry_RepairItem_OVR.setFormat(new BizDataFormat(
				"$number$"));
		...
		prmtRepairItem.addSelectorListener(new SelectorListener() {
			...
			public void willShow(SelectorEvent e) {
				try {
					repairItemEntryF7ListUI = new RepairItemEntryListUIPIEx();
					FilterInfo filterInfo = getVehicleFilterInfo();

					BrandInfo brandInfo = (BrandInfo) prmtBrand.getValue();
					if (brandInfo != null) {
						FilterInfo filterBrand = new FilterInfo();
						filterBrand.getFilterItems().add(
								new FilterItemInfo("parent.brand.id", brandInfo
										.getString("id")));
						filterBrand.getFilterItems().add(
								new FilterItemInfo("orgUnit.id", orgUnitInfo
										.getString("id")));
			...
			public void dataChanged(DataChangeEvent eventObj) {
				...
				if (null != eventObj && null != eventObj.getNewValue()) {
					...
					if (newObject instanceof Object[]) {
						...
						System.arraycopy(((Object) (objs)), 0,
								repairItemEntryInfos, 0, objs.length);
					} else {
						return;
					}
					if (repairItemEntryInfos == null
							|| repairItemEntryInfos.length <= 0)
						return;
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == 10) {
					...
					try {
						repairItemEntryF7ListUI = new RepairItemEntryListUIPIEx();
						repairItemEntryF7ListUI.setSingleSelect(false);
						repairItemEntryF7ListUI
								.setParentNumber(repairtItemWhereLikeStr);
						FilterInfo filterInfo = getVehicleFilterInfo();
						...
						if (brandInfo != null) {
							FilterInfo filterBrand = new FilterInfo();
							filterBrand.getFilterItems().add(
									new FilterItemInfo("parent.brand.id",
											brandInfo.getString("id")));
							filterBrand.getFilterItems().add(
						...
						repairItemEntryF7ListUI.setExpandFilter(filterInfo);
						repairItemEntryF7ListUI.setF7Use(true, null);
						KDTextField txtSearch = (KDTextField) InvokeUtils
								.getFieldValue(repairItemEntryF7ListUI,
										"fastSearchTextField");
					...
					if (null != object) {
						...
						prmtRepairItem_dataChange(kdtRWOItemSpEntry,
								repairItemEntryInfos);
	...
	private RepairItemSpEntryCollection getRepairItemSPEntryCollection(
			RepairItemInfo repairItemInfo) throws Exception {
		IRepairItemSpEntry repairItemSpEntry = RepairItemSpEntryFactory
				.getRemoteInstance();
	...
	private void prmtMaterial_dataChange(DataChangeEvent eventObj) {
		...
		if (null != eventObj && null != eventObj.getNewValue()) {
			...
			if (newObject instanceof Object[]) {
				...
				System.arraycopy(((Object) (objs)), 0, materialInfos, 0,
						objs.length);
			} else {
				return;
			}
			...
			try {
				if (!canChangeBillHeadValue()) {
					MsgBoxEx.showInfo(RepairWOEditUIPIEx.this,
							"已全部结算，不允许添加新项目！");
					prmtMaterial.setValue(null);
					SysUtil.abort();
	...
	private void prmtRepairItem_dataChange(KDTable kdTable,
		...
		for (int length = repairItemEntryInfos.length; i < length; i++) {
			...
			try {
				...
				BigDecimal defaultRepairItemTaxPrice = repairItemInfo
						.getBigDecimal("price"); // 参考售价(含税)
				...
				// 维修项目分录ID
				String repairItemEntryId = ((RepairWORWOItemSpEntryInfo) kdTable
						.getRow(rowIndex).getUserObject()).getString("id");
				rowIndex++;
				// 自动带出关联配件
				if (!PublicUtils.isEmpty(repairItemSpEntryCol)) {
					...
					for (int j = 0; j < repairItemSpEntryCol.size(); j++) {
						...
						kdTable.getCell(rowIndex, "discountRate").setValue(
								retailDiscountRate);
	 ...
	 * @throws Exception
	 */
	private BigDecimal getRepairItemWorkTimeQty(RepairItemInfo repairItemInfo)
			throws Exception {
		if (repairItemInfo == null)
			return BIGDEC0;
		String sql = String
				.format(
						"select isnull(FStdWorkTime,0) FStdWorkTime from T_ATS_RepairItemEntry where FParentId='%s'",
						repairItemInfo.getString("id"));
		IRowSet rs = DBUtils.executeQuery(null, sql);
		if (rs != null && rs.next()) {
	 ...
	 * @return
	 * @throws Exception
	 *             private BigDecimal getWorkTimePrice(BrandInfo brandInfo)
	 *             throws Exception { if (brandInfo == null) return BIGDEC0;
	 *             String sql = String.format("select isnull(FWorkTimeStandardPrice,0) FWorkTimeStandardPrice from T_ATS_WorkTimeStdPrice where FIsUse=1 and FAuditStatus=2 and FBrandID='%s'"
	 *             ,brandInfo.getString("id")); IRowSet rs =
	 *             DBUtils.executeQuery(null, sql); if (rs != null && rs.next())
	 *             { return rs.getBigDecimal("FWorkTimeStandardPrice"); } return
	 *             BIGDEC0;
	 * 
	 *             }
	 */
		...
		prmt.addSelectorListener(new SelectorListener() {
			...
			public void willShow(SelectorEvent e) {
				try {
					repairItemEntryF7ListUI = new RepairItemEntryListUIPIEx();
					repairItemEntryF7ListUI
							.setExpandFilter(getVehicleFilterInfo());
					repairItemEntryF7ListUI.setSingleSelect(false);
					repairItemEntryF7ListUI.setF7Use(true, null);
			...
			public void dataChanged(DataChangeEvent eventObj) {
				if (null != eventObj && null != eventObj.getNewValue()) {
					...
					for (int length = repairItemEntryInfos.length; i < length; i++) {
						int rowIndex = beginRowIndex + i;
						RepairItemEntryInfo repairItemEntryInfo = repairItemEntryInfos[i];
						RepairItemInfo repairItemInfo = repairItemEntryInfo
								.getParent();
						if (isFirstRowIndex) {
						...
						kdTable.getCell(rowIndex, "repairItem").setValue(
								repairItemInfo);
						kdTable.getCell(rowIndex, "t").setValue(TEnum.L);
						kdTable.getCell(rowIndex, "itemspNum").setValue(
								repairItemInfo.getNumber());
	...
	private void registePrmtMaterialF7() {
		...
		if (orgUnitInfo != null) {
			prmtMaterial
					.setQueryInfo("com.kingdee.eas.basedata.master.material.app.RepairMaterialF7Query");
			String mergeColumnKeys[] = { "number", "name", "model", "helpCode",
					"baseUnit.name", "baseQty" };
			...
			setMaterialF7Filter();
			prmtMaterial.setEntityViewInfo(sparepartEntry_Material_PromptBox
					.getEntityViewInfo());
			...
			prmtMaterial.addCommitListener(new CommitListener() {
				public void willCommit(CommitEvent e) {
					...
					FilterItemCollection filterItemCollection = orgUnitFilter
							.getFilterItems();
					filterItemCollection.add(new FilterItemInfo(
							"SaleOrgUnit.id", orgUnitInfo.getId()));
					filterItemCollection.add(new FilterItemInfo("status",
							new Integer(UsedStatusEnum.APPROVED.getValue())));
					FilterInfo vehicleFilterInfo = getVehicleFilterInfo();
					FilterInfo megerFilterInfo = new FilterInfo();
					if (orgUnitFilter != null
							&& !StringUtils.isEmpty(orgUnitFilter.toString()))
						try {
					...
					if (vehicleFilterInfo != null
							&& !StringUtils.isEmpty(vehicleFilterInfo
									.toString()))
						try {
							megerFilterInfo.mergeFilter(vehicleFilterInfo,
									"AND");
						} catch (Exception exc) {
							handUIException(exc);
						}
	...
	private KDBizPromptBox getMaterialPrmt() {
		...
		if (orgUnitInfo != null) {
			...
			prmt.setEnabledMultiSelection(true);
			setMaterialF7Filter();
			prmt.setEntityViewInfo(sparepartEntry_Material_PromptBox
					.getEntityViewInfo());
			...
			prmt.addDataChangeListener(new DataChangeListener() {
				...
				public void dataChanged(DataChangeEvent eventObj) {
					if (null != eventObj && null != eventObj.getNewValue()) {
						...
						if (null == kdTable)
							return;
						KDTSelectManager selectManager = kdTable
								.getSelectManager();
						KDTSelectBlock selectBlock = selectManager.get(0);
						int beginRowIndex = 0;
						if (selectBlock == null)
							...
							beginRowIndex = selectBlock.getBeginRow();
						boolean isFirstRowIndex = true;
	...
	private void initItemSpMaterialF7(IColumn column, ICell cell) {
		...
		if (prmt != null) {
			com.kingdee.bos.ctrl.swing.KDPromptBox.DefaultEditor editor = (com.kingdee.bos.ctrl.swing.KDPromptBox.DefaultEditor) prmt
					.getEditor();
	...
	protected void beforeStoreFields(ActionEvent arg0) throws Exception {
		...
		// throw new
		// com.kingdee.eas.common.EASBizException(com.kingdee.eas.common
		// .EASBizException.CHECKBLANK,new Object[] {"送修人电话"});
		// }
		if (com.kingdee.bos.ui.face.UIRuleUtil
				.isNull(txtRepairSender.getText())) {
		...
		if (com.kingdee.bos.ui.face.UIRuleUtil.isNull(pkIntendDeliveryTime
				.getValue())) {
			throw new com.kingdee.eas.common.EASBizException(
					com.kingdee.eas.common.EASBizException.CHECKBLANK,
					new Object[] { "预计交车时间" });
		...
		if (prmtVehicle.getValue() != null
				&& ((VehicleInfo) prmtVehicle.getValue()).getPlateNum() == null
				&& ((VehicleInfo) prmtVehicle.getValue()).getVIN() == null) {
			MsgBox.showInfo("请维护车辆信息，车牌号码和底盘号至少要填写一项！");
			SysUtil.abort();
		}
		if (pkComeTime.getValue() != null
				&& pkIntendDeliveryTime.getValue() != null) {
			...
			if (returnDate.after(deliveryDate)) {
				MsgBox.showInfo(EASResource.getString(
						"com.kingdee.eas.auto4s.rsm.rs.client.RsResource",
						"NO_LESS_COMETIME"));
				SysUtil.abort();
			}
		}
		...
		for (int i = 0; i < count; i++) {
			if (UIRuleUtil.getBooleanValue(kdtRWOSparepartEntry.getCell(i,
					"IsDelete").getValue()))
				continue;
			if (UIRuleUtil.getBigDecimal(
					kdtRWOSparepartEntry.getCell(i, Cell_Qty).getValue())
					.compareTo(BigDecimal.valueOf(0L)) == -1) {
				// MsgBox.showInfo(EASResource.getString(
				// "com.kingdee.eas.auto4s.rsm.rs.client.RsResource"
				// ,"NOT_QTYLESSTHANZERO"));
			...
			if (UIRuleUtil.getBigDecimal(
					kdtRWOSparepartEntry.getCell(i, Cell_TaxRate).getValue())
					.compareTo(BigDecimal.valueOf(0L)) == -1) {
				MsgBox.showInfo(EASResource.getString(
						"com.kingdee.eas.auto4s.rsm.rs.client.RsResource",
						"NOT_TAXRATELESSTHANZERO"));
				SysUtil.abort();
				continue;
			}
			if (UIRuleUtil.getBigDecimal(
					kdtRWOSparepartEntry.getCell(i, Cell_TaxPrice).getValue())
					.compareTo(BigDecimal.valueOf(0L)) == 0
					|| UIRuleUtil.getBigDecimal(
							kdtRWOSparepartEntry.getCell(i, Cell_TaxPrice)
									.getValue()).compareTo(
							BigDecimal.valueOf(0L)) == -1) {
				// MsgBox.showInfo(EASResource.getString(
				// "com.kingdee.eas.auto4s.rsm.rs.client.RsResource"
				// ,"NOT_TAXPRICELESSTHANZERO"));
				// SysUtil.abort();
				// continue;
			}
			if (UIRuleUtil
					.getBigDecimal(
							kdtRWOSparepartEntry.getCell(i, Cell_NoIssueQty)
									.getValue()).compareTo(
							BigDecimal.valueOf(0L)) == -1) {
				// MsgBox.showInfo(EASResource.getString(
				// "com.kingdee.eas.auto4s.rsm.rs.client.RsResource"
		...
		for (int i = 0; i < attachmentCount; i++)
			if (UIRuleUtil.getBigDecimal(
					kdtRWOAttachmentItemEntry.getCell(i, Cell_DiscountRate)
							.getValue()).compareTo(BigDecimal.valueOf(100L)) == 1) {
		...
		if (kdtRWORepairItemEntry.getRowCount() > 0) {
			for (int i = 0; i < kdtRWORepairItemEntry.getRowCount(); i++) {
				...
				if (kdtRWORepairItemEntry.getCell(i, "IsReturnRepair")
						.getValue().equals(Boolean.TRUE)) {
					kdtRWORepairItemEntry.getCell(i, "ReworkReason")
							.getStyleAttributes().setLocked(false);
					kdtRWORepairItemEntry.getCell(i, "ReworkReason")
							.getStyleAttributes().setBackground(YELLOW);
					if (row.getCell("ReworkReason").getValue() == null)
						throw new EASBizException(EASBizException.CHECKBLANK,
								new Object[] { kdtRWORepairItemEntry.getHead()
										.getRow(0).getCell(
												row.getCell("ReworkReason")
														.getColumnIndex())
										.getValue() });
				} else {
					kdtRWORepairItemEntry.getCell(i, "ReworkReason")
		...
		for (int i = 0; i < item; i++) {
			...
			if (row.getCell("RepairItem").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWORepairItemEntry.getHead()
								.getRow(0).getCell(
										row.getCell("RepairItem")
												.getColumnIndex()).getValue() });
			if (row.getCell("PaymentClassify").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWORepairItemEntry.getHead()
								.getRow(0).getCell(
										row.getCell("PaymentClassify")
												.getColumnIndex()).getValue() });
			if (row.getCell("SettleObject").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWORepairItemEntry.getHead()
								.getRow(0).getCell(
										row.getCell("SettleObject")
												.getColumnIndex()).getValue() });
			if (row.getCell("RepairClassify").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWORepairItemEntry.getHead()
								.getRow(0).getCell(
										row.getCell("RepairClassify")
												.getColumnIndex()).getValue() });
			if (!paramValue_RSM012
					&& UIRuleUtil.getBigDecimal(
							kdtRWORepairItemEntry.getCell(i, Cell_DiscountRate)
									.getValue()).compareTo(
							BigDecimal.valueOf(100L)) == 1) {
				MsgBox.showInfo("维修项目折扣率不能大于100！");
				SysUtil.abort();
			}
			if ("保险".equals(row.getCell("PaymentClassify").getValue()
					.toString())
					&& prmtInsuranCompany.getValue() == null
			...
			if ("保修".equals(row.getCell("PaymentClassify").getValue()
					.toString())
					&& prmtWarrantyType.getValue() == null) {
				MsgBox.showInfo(EASResource.getString(
						"com.kingdee.eas.auto4s.rsm.rs.client.RsResource",
						"NOT_WARRANTY"));
		...
		if (!paramValue_RSM012) {
			/*
			 * for (int i = 0; i < item; i++) { IRow row =
			 * kdtRWORepairItemEntry.getRow(i); if
			 * (!UIRuleUtil.getBooleanValue(kdtRWORepairItemEntry.getCell(i,
			 * "IsDelete").getValue())&&
			 * UIRuleUtil.getBigDecimal(row.getCell(Cell_ARAmount
			 * ).getValue()).compareTo(BIGDEC0) == -1) { MsgBox.showInfo((new
			 * StringBuilder()).append("维修项目分录第 ").append(i +
			 * 1).append(" 行应收金额只能大于等于0").toString()); SysUtil.abort(); } }
			 */
		}
		for (int i = 0; i < sp; i++) {
			...
			if (row.getCell("Material").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWOSparepartEntry.getHead().getRow(0)
								.getCell(
										row.getCell("Material")
												.getColumnIndex()).getValue() });
			if (row.getCell("PaymentClassify").getValue() == null)
				throw new EASBizException(EASBizException.CHECKBLANK,
						new Object[] { kdtRWOSparepartEntry.getHead().getRow(0)
			...
			if (row.getCell(Cell_Qty).getValue() == null)
				throw new EASBizException(
						EASBizException.CHECKBLANK,
						new Object[] { kdtRWOSparepartEntry
								.getHead()
								.getRow(0)
								.getCell(row.getCell(Cell_Qty).getColumnIndex())
								.getValue() });
			if (row.getCell(Cell_Unit).getValue() == null)
				throw new EASBizException(
						...
						new Object[] { kdtRWOSparepartEntry
								...
								.getRow(0)
								.getCell(
										row.getCell(Cell_Unit).getColumnIndex())
								.getValue() });
			if ("保险".equals(row.getCell("PaymentClassify").getValue()
					.toString())
					&& prmtInsuranCompany.getValue() == null
	...
	private void checkRepairspCT() throws Exception {
		...
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			...
			if (qty.compareTo(BIGDEC0) >= 0) {
				msg.append(String.format("第%d行，[%s]拆退数量不能大于0", i + 1, spName))
						.append(CR);
			}
		}
		if (msg.length() > 0) {
			MsgBoxEx
					.showDetailAndOK(this, "拆退数量异常", msg.toString(), MsgBox.YES);
			abort();
		}
	}
	...
	private void checkReparirSpCostAndInvQty() throws Exception {
		...
		if (hasPermission_OprtRetailBelowCost)
			return;
		String orgId = orgUnitInfo.getString("id");
		StringBuilder msg = new StringBuilder();
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
		...
		if (msg.length() > 0) {
			MsgBoxEx.showDetailAndOK(this, "折后价低于成本价", msg.toString(),
					MsgBox.YES);
			abort();
		}
	}
	...
	public SelectorItemCollection getSelectors() {
		...
		sic.add(new SelectorItemInfo("RWOItemSpEntry.w.typeCode"));
		sic.add(new SelectorItemInfo("CustomerAccount.typeCode"));
		return sic;
	...
	private void saveRepairItemAndSpToEntry() throws Exception {
		...
		for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
			...
			BigDecimal discountRate = (BigDecimal) row.getCell("discountRate")
					.getValue(); // 折扣
			BigDecimal taxRate = (BigDecimal) row.getCell("taxRate").getValue(); // 税率
			BigDecimal taxPrice = price.multiply(BIGDEC1.add(taxRate.divide(
					BIGDEC100, 10, BigDecimal.ROUND_HALF_UP))); // 含税单价
			...
			if ("L".equals(tType)) { // 项目
				...
				if (rowRepairItem == null) {
					addLine(kdtRWORepairItemEntry);
					rowRepairItem = kdtRWORepairItemEntry
							.getRow(kdtRWORepairItemEntry.getRowCount() - 1);
					hashRepairItem.put(itemSpEntryId, rowRepairItem);
					isChangeRepairItem = true;
					rowRepairItem.getCell("itemSpEntryId").setValue(
							itemSpEntryId);
				} else {
					RepairItemInfo oldRepairItemInfo = (RepairItemInfo) rowRepairItem
							.getCell("RepairItem").getValue();
				...
				if (isChangeRepairItem) { // 标准工时等，重新付值
					...
					rowRepairItem.getCell("RepairItem")
							.setValue(repairItemInfo);
					rowRepairItem.getCell("RepairItemName").setValue(
							repairItemInfo.getName());
					rowRepairItem.getCell("ItemRemark").setValue(
							repairItemInfo.getName());
				}
				...
				// 调用标准产品-更新计算金额
				rowRepairItem.getCell("SettleObject").setValue(null); // 需先将结算对象置空
				// ，
				// 否则kdtWorkTimeEntrycount
				// ，
				// 会根据结算对象重取工时单价
				kdtWorkTimeEntrycount(rowRepairItem.getRowIndex());
				rowRepairItem.getCell("SettleObject").setValue(
						row.getCell("settlementObject").getValue());

				rowRepairItem.getCell("RepairClassify").setValue(
						defaultRepairClassifyInfo);
				...
				if (rowSp == null) {
					addLine(kdtRWOSparepartEntry);
					rowSp = kdtRWOSparepartEntry.getRow(kdtRWOSparepartEntry
							.getRowCount() - 1);
					hashSp.put(itemSpEntryId, rowSp);
					isChangeRepairItem = true;
					rowSp.getCell("itemSpEntryId").setValue(itemSpEntryId);
				} else {
					MaterialInfo oldMaterialInfo = (MaterialInfo) rowSp
							.getCell("Material").getValue();
				...
				rowSp.getCell("RepairClassify").setValue(
						defaultRepairClassifyInfo);

				BigDecimal issueQty = UIRuleUtil.getBigDecimal(rowSp.getCell(
						Cell_IssueQty).getValue());
				rowSp.getCell(Cell_NoIssueQty).setValue(qty.subtract(issueQty));
				rowSp.getCell("SettleObject").setValue(
						row.getCell("settlementObject").getValue());
	...
	public void calItemSpEntryAmount(IRow row) throws Exception {
		...
		BigDecimal qty = PublicUtils.getBigDecimal(row.getCell("qty")
				.getValue()); // 数量/工时
		BigDecimal price = PublicUtils.getBigDecimal(row.getCell("price")
				.getValue()); // 不含单价
		BigDecimal discountRate = PublicUtils.getBigDecimal(row.getCell(
		...
		BigDecimal taxPrice = price.multiply(BIGDEC1.add(taxRate.divide(
				BIGDEC100, 10, BigDecimal.ROUND_HALF_UP))); // 含税单价
		BigDecimal taxAmount = qty.multiply(taxPrice).multiply(
				BIGDEC1.subtract(discountRate.divide(BIGDEC100, 10,
						BigDecimal.ROUND_HALF_UP)));// 含税总计
		...
		if (PublicUtils.equals(TEnum.L, tType)) { // 项目行
			...
			BigDecimal issueQty = (BigDecimal) row.getCell("issueQty")
					.getValue(); // 已出库数
	...
	private void initItemSpRepairItemEntryF7(IColumn column, ICell cell) {
		...
		KDTDefaultCellEditor repairItemDefaultCellEditor = new KDTDefaultCellEditor(
				prmt);
		ObjectValueRender kdtRWORepairItemEntry_RepairItem_OVR = new ObjectValueRender();
		kdtRWORepairItemEntry_RepairItem_OVR.setFormat(new BizDataFormat(
				"$number$"));
	...
	public void initEntryFormat() {
		...
		KDTableUtils
				.formatDecimal(kdtRWORepairItemEntry, "WorkTimePrice", true);
		KDTableUtils.formatDecimal(kdtRWORepairItemEntry, "WorkTimeStdPrice",
				true);
		KDTableUtils.formatDecimal(kdtRWORepairItemEntry, Cell_WorkTimeAmount,
				true);
		KDTableUtils.formatDecimal(kdtRWORepairItemEntry, "AssignWorkTime",
				true);
		...
		KDTableUtils.formatDecimal(kdtRWORepairItemEntry, Cell_ActualWorkTime,
				true);
		...
		KDTableUtils
				.formatDecimal(kdtRWOSparepartEntry, Cell_NoTaxAmount, true);
		KDTableUtils.formatDecimal(kdtRWOSparepartEntry, Cell_IssueQty, true);
		KDTableUtils.formatDecimal(kdtRWOAttachmentItemEntry, "AttaItemAmount",
				true);
		KDTableUtils.formatDecimal(kdtRWOAttachmentItemEntry, Cell_ARAmount,
				true);
		KDTableUtils.formatDecimal(kdtRWOAttachmentItemEntry,
				Cell_DiscountAmount, true);
		...
		KDTableUtils.formatDecimal(kdtRWOItemSpEntry, "allocateExenseRate",
				false);
	...
	public void setLoadField() {
		...
		if (isFirstGetInsFromVehicle && vehicleInfo != null) {
			...
			sql
					.append(
							"select b.FID BrandID,b.FName_l2 BrandName,b.FNumber BrandNumber,a.FPlateDate,")
					.append(
					...
					.append(
							String.format("where a.FID='%s'", vehicleInfo
									.getString("id")));
			...
			try {
				...
				if (rs != null && rs.next()) {
					...
					String strCustomerInfo = rs.getString("CustomerName") + " "
							+ rs.getString("CustomerPhone") + CR
					...
					// 按品牌带出默认维修项目TXT && 维修项目-代金券
					defaultRepairItemForTXT = GAUtils
							.getDefaultRepairItemForTXT(null, brandInfo,
									orgUnitInfo);
					// defaultRepairItemForDJQ =
					// GAUtils.getDefaultRepairItemForDJQ(null,
					// brandInfo,orgUnitInfo);
	...
	protected void mile() {
		...
		if (info.getVehicle() != null) {
			...
			filterInfo.getFilterItems().add(
					new FilterItemInfo("Vehicle", info.getVehicle().getId()
							.toString()));
			Date comeTime = new Date();
			if (pkComeTime.getValue() != null)
				comeTime = (Date) pkComeTime.getValue();
			filterInfo.getFilterItems().add(
					new FilterItemInfo("ComeTime", comeTime, CompareType.LESS));
			filterInfo.getFilterItems().add(
					new FilterItemInfo("status", RepairBillStatusEnum.ABOLISH,
			...
			if (txtMile.getValue() != null) {
				...
				if (repairWO != null
						...
						&& UIRuleUtil.getBigDecimal(info.getMile()).compareTo(
								UIRuleUtil.getBigDecimal(repairWO.get(0)
										.getMile())) <= 0 && isVerifyMile) {
					int status = MsgBox
							.showConfirm2("进厂行驶里程小于或等于上次进厂行驶里程,是否继续?");
					if (2 == status)
						SysUtil.abort();
					isVerifyMile = false;
				}
	...
	public void actionIsShowStdItemspEntry_actionPerformed(ActionEvent e)
		...
		if (menuItemIsShowStdItemspEntry.isSelected()) {
			kDTRWOPane.insertTab(resHelper.getString("kDPRwoItem.constraints"),
					null, kDPRwoItem, resHelper
							.getString("kDPRwoItem.constraints"), 1);
			kDTRWOPane.insertTab(resHelper.getString("kDPRwoSp.constraints"),
					null, kDPRwoSp,
					resHelper.getString("kDPRwoSp.constraints"), 2);
		} else {
			kDTRWOPane.remove(kDPRwoItem);
			kDTRWOPane.remove(kDPRwoSp);
		}
	...
	public void saveVehicleMile() throws Exception {
		...
		try {
			...
			if (rs != null && rs.next()) {
				warrRemindInfo = warrRemind.getWarrRemindInfo(new ObjectUuidPK(
						rs.getString(1)));
			} else {
	...
	private void saveRemark() throws Exception {
		if (PublicUtils.isEmpty(editData.getString("id")))
			return;
		if (PublicUtils.isEmpty(txtRemark.getText().trim()))
			return;
		IVehicleRepairRemark repairRemark = VehicleRepairRemarkFactory
				.getRemoteInstance();
		VehicleRepairRemarkInfo repairRemarkInfo = null;
		try {
			repairRemarkInfo = repairRemark
					.getVehicleRepairRemarkInfo(String.format(
							"where repairwo.id='%s'", editData.getString("id")));
		} catch (Exception e) {
	...
	/*
	 * private void saveRepairSender() throws Exception { if
	 * (PublicUtils.isEmpty(editData.getString("id"))) return; IContactPerson
	 * contactPerson = ContactPersonFactory.getRemoteInstance(); String
	 * repairSender = txtRepairSender.getText(); String repairTel =
	 * txtTel.getText(); ContactPersonInfo cPersonInfo = new
	 * ContactPersonInfo(); try { cPersonInfo =
	 * contactPerson.getContactPersonInfo
	 * (String.format("where name='%s'",repairSender.trim())); } catch
	 * (Exception e) { cPersonInfo = new ContactPersonInfo(); }
	 * cPersonInfo.setName(repairSender);
	 * cPersonInfo.setContactMobile(repairTel); contactPerson.save(cPersonInfo);
	 * 
	 * 
	 * }
	 */
	...
	private void checkDataChange() throws Exception {
		...
		if (editData != null && !PublicUtils.isEmpty(editData.getString("id"))) {
			String sql = String
					.format(
							"select FLastUpdateTime from T_ATS_RepairWO where fid='%s'",
	...
	protected void btnShowRepairSender_actionPerformed(ActionEvent e)
		...
		ServiceOrgUnitInfo seriveOrgInfo = (ServiceOrgUnitInfo) prmtOrgUnit
				.getValue();
		boolean hasPermission_RepairManView = PermUtils.hasFunctionPermission(
				null, userInfo, seriveOrgInfo, "repairMan_View");
		...
		// IUIWindow iUIWindowSizesHEdit =
		// iUIFactory.create(RepairSenderF7UI.class.getName(),uictx); //
		// 获取FeedRecListUI的IUIWindow
		// RepairSenderF7UI f7 = (RepairSenderF7UI)
		// iUIWindowSizesHEdit.getUIObject();
		IUIWindow iUIWindowSizesHEdit = iUIFactory.create(RepairManF7UI.class
				.getName(), uictx); // 获取FeedRecListUI的IUIWindow
		...
		if (repairManInfo != null && txtRepairSender.isEditable()) {
			...
			String strCustomerInfo = StringUtils
					.cnulls(repairManInfo.getName())
					+ " "
					+ StringUtils.cnulls(repairManInfo.getTel())
					+ CR
					+ StringUtils.cnulls(repairManInfo.getAddr());
			txtcustomInfo.setText(strCustomerInfo);
		}
	...
	protected void initWorkButton() {
		...
		KDLayout.Constraints c = new KDLayout.Constraints(r.x, r.y, r.width,
				r.height, KDLayout.Constraints.ANCHOR_TOP
		...
		kDPRwoItemSp.add(kdtRWOItemSpEntry_detailPanel.getRemoveLinesButton(),
				new com.kingdee.bos.ctrl.swing.KDLayout.Constraints(
						r.width - 32, 5, 22, 19, 9));
		...
		kDPRwoItemSp.add(btnSplitLine,
				new com.kingdee.bos.ctrl.swing.KDLayout.Constraints(
						r.width - 59, 5, 22, 19, 9));
	...
	public void actionSplitLine_actionPerformed(ActionEvent e) throws Exception {
		...
		// kdtRWOItemSpEntry.getColumn("originalEntryId").getStyleAttributes().
		// setHided(false);
	...
	public void actionPrint_actionPerformed(ActionEvent e) throws Exception {
		...
		if (idList == null || idList.size() == 0 || getTDQueryPK() == null
			...
			PrintIntegrationManager.initPrint(appHlp, editData.getBOSType(),
					idList, getTDFileName(),
					"com.kingdee.eas.scm.common.SCMResource", true);
			appHlp.print(getTDFileName(), multiDataSourceProviderProxy,
	...
	public void actionPrintPreview_actionPerformed(ActionEvent e)
		...
		if (idList == null || idList.size() == 0 || getTDQueryPK() == null
			...
			PrintIntegrationManager.initPrint(appHlp, editData.getBOSType(),
					idList, getTDFileName(),
					"com.kingdee.eas.scm.common.SCMResource", true);
			appHlp.printPreview(getTDFileName(), multiDataSourceProviderProxy,
					SwingUtilities.getWindowAncestor(this));
	...
	private PrintIntegrationInfo getPrintIntegrationInfo() throws Exception {
		...
		PrintIntegrationInfo piInfo = pi.getBillPrintInfo(editData.getBOSType()
				.toString(), editData.getString("id"));
		return piInfo;
	}

	private void setSettlePrintFlag(String printedTemplatePath,
			PrintIntegrationInfo oldPIInfo, PrintIntegrationInfo newPIInfo)
			throws Exception {
		if (newPIInfo == null)
			return;
	...
	//		return;
		...
		if (oldPIInfo == null) {
			...
			if (oldPIInfo.getPrintedNumber() == newPIInfo.getPrintedNumber())
				return;
		}

		String settlePrintedTemplatePath = ParamUtils.getParamValue(null,
				"GA001", editData.getOrgUnit().getString("id"));
		String[] printedTemplatePaths = settlePrintedTemplatePath.split(";");

		if (!PublicUtils.contain(printedTemplatePaths, printedTemplatePath))
			return;
	...
	private boolean canChangeBillHeadValue() throws Exception {
		...
		if (PublicUtils.equals(gaBillStatus.getSelectedItem(),
			...
			for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
				...
				BigDecimal taxAmount = PublicUtils.getBigDecimal(row.getCell(
						"taxAmount").getValue());
				if (taxAmount.compareTo(BIGDEC0) != 0)
					return false;
			}
		}
	...
	private boolean canChangeVehicle() throws Exception {
		if (PublicUtils.equals(gaBillStatus.getSelectedItem(),
			...
			for (int i = 0; i < kdtRWOItemSpEntry.getRowCount(); i++) {
				...
				IEnum iType = (IEnum) row.getCell("i").getValue();
				if (taxAmount.compareTo(BIGDEC0) != 0
						&& PublicUtils.equals(IEnum.X, iType)) {
					return false;
				}
			}
	...
	public void actionCreateTo_actionPerformed(ActionEvent e) throws Exception {
		 ...
		 * 更新外包价格
		 */
		String sql = String.format("update CT_ATS_RepairWORWOItemSpEntry set cfwprice=0  where FParentID='%s'", id);
		DBUtils.execute(null, sql);
		sql = String.format("UPDATE CT_ATS_RepairWORWOItemSpEntry " + 
					"SET CT_ATS_RepairWORWOItemSpEntry.cfwprice = w.cfprice " +
					"FROM T_ATS_RepairWO r,CT_ATS_Gyswbprice w " +
					"WHERE	CT_ATS_RepairWORWOItemSpEntry.fparentid = r.fid " +
	...
	@Override
	protected void prmtBrand_dataChanged(DataChangeEvent e) throws Exception {
