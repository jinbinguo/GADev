...
public class TreeDatabaseEditUI extends AbstractTreeDatabaseEditUI {
	private static final Logger logger = CoreUIObject.getLogger(TreeDatabaseEditUI.class);
	private boolean isVerifyInput = false;
	
	public TreeDatabaseEditUI() throws Exception {
		super();
	}
	...
	protected void initDataStatus() {
		...
		if (isViewStatus() && getDataObject().get("deletedStatus") != null
				&& getDataObject().getInt("deletedStatus") == 1) {
			actionCancel.setEnabled(true);
			actionCancelCancel.setEnabled(false);
		} else if (isViewStatus()
				&& getDataObject().get("deletedStatus") != null
				&& getDataObject().getInt("deletedStatus") == 2) {
		...
		DeletedStatusEnum deletedStatus = (DeletedStatusEnum) boxDeletedStatus
				.getSelectedItem();
	...
	public void actionEdit_actionPerformed(ActionEvent e) throws Exception {
		...
		if (editData.isScheduled()) {
			String msg = EASResource
					.getString("com.kingdee.eas.basedata.scm.common.SCMResource.SystemSettingEdit");
			MsgBox.showInfo(this, msg);
			return;
		}
		/** 启用状态时不允许编辑 */
		if (editData.getDeletedStatus().equals(DeletedStatusEnum.OPEN)) {
			MsgBox.showInfo(this, "已启用数据不允许编辑!");
			return;
		}
	...
	public void actionRemove_actionPerformed(ActionEvent e) throws Exception {
		...
		if (editData.isScheduled()) {
			String msg = EASResource
					.getString("com.kingdee.eas.basedata.scm.common.SCMResource.SystemSettingDel");
			MsgBox.showInfo(this, msg);
			return;
		}
		/** 启用状态基础资料不允许删除 */
	...
	/**
	 * 改造removeByPK(IObjectPK),由业务建模工具生成的代码removeByPK(IObjectPK)有自动回收编码，但不在同一事务，
	 * 所以改造到后台执行，放于同一事务 代码与removeByPK(IObjectPK)一致，未有改动，因继承关系的原因，所以必须得拷贝出来另写
	 * 
	 * @param pk
	 * @throws Exception
	 */
			...
			if (value != null) {
				...
				MsgBox
						.showInfo(
								this,
								EASResource
										.getString("com.kingdee.eas.framework.FrameWorkResource.confirm_close"));
				destroyWindow();
			}
			innerLoadFields();
			if (idList.size() == 0)
	...
	protected void verifyInput(ActionEvent e) throws Exception {
		...
		/** 检查名称是否重复 */
		String strName = ((MultiLangItem) txtName.getDefaultLangItemData())
				.getData().toString();
		try {
			((IDataBase) getBizInterface()).checkNameDup(editData);
		} catch (Exception ee) {
			int iResult = MsgBox.showConfirm2(String.format(
					"名称[%s]已重复存在，是否继续保存?", strName));
	...
	public void verifyRequireComp() throws Exception {
		...
		if (!CodingRuleUtils.hasCodingRule(editData, null)
				&& StringUtils.isEmpty(txtNumber.getText())) {
			txtNumber.requestFocus();
			throw new EASBizException(
					new NumericExceptionSubItem("", "编码不能为空！"));
		}
		...
		while (itSet.hasNext()) {
			...
			if (!rqCompInfo.isTable()) { // 检查表头必录项
				UIUtils
						.checkRequireCompValue((JComponent) rqCompInfo
								.getComp());
			} else {
				KDTable table = rqCompInfo.getTable();
				String columName = rqCompInfo.getColName();
				String tblName = table.getName();
				if (hashTable.get(tblName) == null)
					hashTable.put(tblName, table);
		...
		try {
			...
			for (int i = 0; vecProperty != null && i < vecProperty.size(); i++) {
				...
				if (!newData.containsKey(rfi.getPropertyName())) {
					MsgBoxEx.showDetailAndOK(null, "复制异常，字段标识有误", String
							.format("字段标识[%s]有误,请开发检查代码!", rfi
									.getPropertyName()), MsgBox.OK);
					SysUtil.abort();
				}
			...
			for (int i = 0; entrysName != null && i < entrysName.length; i++) {
				...
				if (!newData.containsKey(entryName)) {
					MsgBoxEx.showDetailAndOK(null, "复制异常，字段标识有误", String
							.format("分录标识[%s]有误,请开发检查代码!", entryName),
							MsgBox.OK);
					SysUtil.abort();
				}

				Vector<ResetFieldInfo> vecEntryColumn = resetFieldCol
						.getEntryColumn(entryName);
				IObjectCollection entryCol = (IObjectCollection) newData
						.get(entryName);

				for (int j = 0; entryCol != null && j < entryCol.size(); j++) {
					IObjectValue entryInfo = entryCol.getObject(j);
					for (int k = 0; vecEntryColumn != null
							&& k < vecEntryColumn.size(); k++) {
						ResetFieldInfo rfi = vecEntryColumn.get(k);
						if (!entryInfo.containsKey(rfi.getColumnFlagName())) {
							MsgBoxEx.showDetailAndOK(null, "复制异常，字段标识有误",
									String.format("分录[%s]中列标识[%s]有误,请开发检查代码!",
											rfi.getEntryName(), rfi
													.getColumnFlagName()),
									MsgBox.OK);
							SysUtil.abort();
						}
						entryInfo.put(rfi.getColumnFlagName(), rfi
								.getDefaultValue());
					}
				}
			}
	...
	public void showCancelCancelMessage() {
		showBarMessage("启用成功");
	}
	...
	private void checkPermission(String action) throws Exception {
		if (getControlType().equalsIgnoreCase("S1")) {
			if (!getCurrentCUID().equals(
				...
				.equalsIgnoreCase("S4"))
				&& !action.equals("ACTION_ADDNEW")
				&& !getCurrentCUID().equals(getCUIDFromBizobject()))
			throwsExceptionForNoPermission(action);
	}
	...
	private String getControlType() {
		if (controlType == null) {
			...
			try {
				...
				controlType = BaseDataFactory.getInstance().getControlType(
						getBizType());
			} catch (Exception e) {
	...
	private void innerLoadFields() {
		...
		loadFields();
		restoreDetailTableListener(
				com.kingdee.bos.ctrl.kdf.table.event.KDTPropertyChangeListener.class,
				lsts);
	}
