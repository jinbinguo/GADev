 ...
 * output class name
 */
public class SimpleDatabaseEditUI extends AbstractSimpleDatabaseEditUI {
	private static final Logger logger = CoreUIObject.getLogger(SimpleDatabaseEditUI.class);
	private boolean isVerifyInput = false;

	public SimpleDatabaseEditUI() throws Exception {
		super();
	}
	 ...
	 * 验证必录项
	 */
	private void verifyRequireComp() throws Exception {
		...
		while (itSet.hasNext()) {
			...
			if (!rqCompInfo.isTable()) { //检查表头必录项
				if (rqCompInfo.isPwd()) UIUtils.checkRequireCompValue((JComponent)rqCompInfo.getPwdComp());
				else UIUtils.checkRequireCompValue((JComponent)rqCompInfo.getComp());
			} else {
